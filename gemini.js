...
【ルール】
1. 各スキルの「消費ポイント（cost）」は80以上の整数値で、能力の強さに応じて以下の範囲で評価してください:
   - その他スキル: 80〜190pt
   - バフ・デバフ系（buff_atk, buff_def, buff_hp, buff_spd, debuff_def等）: 110〜190pt（下限110pt）
   - 全スキルの共通上限は190ptです
2. 各スキルは以下のJSON構造で出力してください:
{
  "name": "スキル名",
  "description": "スキルの説明文（1〜2文）",
  "condition": "発動条件（以下のいずれかの形式で指定: 常時, 生存中, 死亡時, バトル開始時, 攻撃開始時, 被弾時, 撃破時, Nターン毎, HP XX%以下, HP XX%以上, Nターン以降, Nターン以内, 先制/第1ターン, 時止め）※「XX」には必ず具体的な数値（例: 30, 50）を入れてください。プレースホルダーは厳禁です。",
  "probability": 発動確率の数値（0〜100）,
  "target": "効果対象（例: 敵, 味方, 自分, 単体, 全体）",
  "targetRange": "効果対象範囲（例: 前衛, 後衛, 単体, 全体, ランダムN体）",
  "effectType": "効果種別（damage, heal, buff_atk, buff_def, buff_hp, buff_spd, debuff_def, damage_up, stun, combo, lifesteal のいずれか）",
  ...
  "cost": 消費ポイント（80〜190の整数、バフ/デバフ系は110〜190）
}
...
6. 【パワーバランス制約】ステータス合計は200ptのため、以下の上限を厳守してください:
   ...
   - buff_atk/buff_def/buff_hp/buff_spd/debuff_def: 効果は対象ステータスの50%まで（percentの場合は50以下）
...
8. 「生存中」条件はパッシブ/アクティブ効果に適しています。「死亡時」条件は道連れ・遺言・復活・ラストディザスター等の効果に適しています。

出力例:
[{"name":"紅蓮の剣舞","description":"炎を纏った剣で全体攻撃","condition":"常時","probability":80,"target":"敵","targetRange":"全体","effectType":"damage","valueType":"flat","effectValue":80,"duration":0,"cost":120},{...},{...}]`;
  ...
  try {
    ...
    candidates = candidates.map(c => {
      ...
      const isBuffType = ['buff_atk', 'buff_def', 'buff_hp', 'buff_spd', 'debuff_def'].includes(effectType);
      let cost = Math.max(80, Math.floor(c.cost || 80));
      if (isBuffType) cost = Math.max(110, cost);
      cost = Math.min(190, cost);
      return {
        name: c.name || '名称不明',
